/*
 * $Id: LabelH.c,v 1.1 1998/01/07 03:49:23 cwikla Exp $
...
/* Labelhack doesn't work with recent versions of Lesstif */
...
static void	initialize(XmLabelHackWidget _request, XmLabelHackWidget _new,
...
XmLabelHackClassRec xmLabelHackClassRec = {
	{
		...
		NULLQUARK,				/* xrm_class */
		TRUE,					/* compr_motion */
		(XtEnum)TRUE,				/* compr_exposur */
		TRUE,					/* compr_enterleav */
		FALSE,					/* visib_interest */
...
)
{
	...
	if (HACK_CLASS(lhwc).on == -1) {
		if (HACK(_new).insensitive3D && CORE(_new).depth > 4) {
			...
			HACK(_new).bottomGC =
				_bottomShadowGC((XmLabelWidget)_new);
		}
...
static GC
_topShadowGC(XmLabelWidget _label)
{
	XGCValues	gcValues;
	XtGCMask	mask;
	XFontStruct	*fs;
...
static GC
_bottomShadowGC(XmLabelWidget _label)
{
	XGCValues	gcValues;
	XtGCMask	mask;
	XFontStruct	*fs;
...
static void
_grabbedLabelExpose(Widget _w, XEvent *_event, Region _region)
{
	GC	insensitiveGC;
...
static GC
_gadgetParentTopShadowGC(XmLabelGadget _label)
{
	XGCValues	gcValues;
	XtGCMask	mask;
	XFontStruct	*fs;
...
static GC
_gadgetParentBottomShadowGC(XmLabelGadget _label)
{
	XGCValues	gcValues;
	XtGCMask	mask;
	XFontStruct	*fs;
...
static void
_grabbedLabelGadgetExpose(Widget _w, XEvent *_event, Region _region)
{
	GC	insensitiveGC;
	...
	if (XtIsSensitive(_w))
		...
		LABEL(label).insensitive_GC =
			_gadgetParentBottomShadowGC(label);
		(*oldLabelGadgetExposeProc)(_w, _event, _region);
		XtReleaseGC(_w, LABEL(label).insensitive_GC);
...
static void
_replaceLabelGadgetExpose(void)
{
...
#endif	/* LesstifVersion NO_LABELH XmVersion */
