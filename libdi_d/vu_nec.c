 ...
 */
#ifndef lint
static char *_vu_nec_c_ident_ = "@(#)vu_nec.c	6.41 04/01/14";
#endif
...
)
{
	...
	if (!ret && addr_fmt & ADDR_MSF) {
		...
		if (ret && !(addr_fmt & ADDR_OPTEND)) {
			...
			ret = pthru_send(devp, DI_ROLE_MAIN,
					 cdb, 10, NULL, 0, NULL, 0,
					 OP_NODATA, 10, TRUE);
		}
	}
 ...
 */
bool_t
nec_get_playstatus(cdstat_t *sp)
{
	...
	if (!pthru_send(devp, DI_ROLE_MAIN, cdb, 10, buf, SZ_VN_RDSUBQ,
			NULL, 0, OP_DATAIN, 5, TRUE))
		return FALSE;

	DBGDUMP(DBG_DEVIO)("nec: Read Subchannel data bytes",
		buf, SZ_VN_RDSUBQ);
...
nec_get_toc(curstat_t *s)
{
	...
	if (!pthru_send(devp, DI_ROLE_MAIN, cdb, 10, buf, SZ_VN_RDTOC, NULL, 0,
			OP_DATAIN, 5, TRUE))
		return FALSE;

	DBGDUMP(DBG_DEVIO)("nec: Read TOC data bytes", buf, SZ_VN_RDTOC);
	...
	for (i = 0, j = (int) s->first_trk; j <= (int) s->last_trk; i++, j++) {
		...
		if (!pthru_send(devp, DI_ROLE_MAIN, cdb, 10, buf, SZ_VN_RDTOC,
				NULL, 0, OP_DATAIN, 5, TRUE))
			return FALSE;

		DBGDUMP(DBG_DEVIO)("nec: Read TOC data bytes",
			buf, SZ_VN_RDTOC);
