 ...
 */
#ifndef lint
static char *_os_aux_c_ident_ = "@(#)os_aux.c	6.48 04/03/02";
#endif
...
)
{
	...
	sc_cmd.datasent	= 0;
	sc_cmd.sensebuf	= (uchar *) sensept;
	sc_cmd.senselen	= (uchar) senselen;

	if (datalen != 0) {
		if (datalen > AUX_MAX_XFER) {
			DBGPRN(DBG_DEVIO)(errfp,
	...
	if (ioctl(devp->fd, SCSISTART, (uchar *) &sc_cmd) < 0) {
...
pthru_open(char *path)
{
	...
	if ((devp->fd = open(path, O_RDONLY, NULL)) < 0) {
		DBGPRN(DBG_DEVIO)(errfp,
...
pthru_vers(void)
{
	return ("OS module for Apple A/UX 3.x");
}
