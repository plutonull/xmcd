 ...
 */
#ifndef lint
static char *_os_aix_c_ident_ = "@(#)os_aix.c	6.57 04/03/02";
#endif
...
)
{
	...
	if ((kstat = ioctl(devp->fd, CDIOCMD, &sc_cmd)) < 0) {
		...
		if (sc_cmd.scsi_bus_status != SC_GOOD_STATUS) {
			if (app_data.scsierr_msg && prnerr) {
				(void) fprintf(errfp,
					"%s: %s %s:\n%s=0x%x %s=0x%x\n",
					APPNAME,
					"SCSI bus status error on", devp->path,
					"Opcode", cmdpt[0],
					"Status", sc_cmd.scsi_bus_status);
			}

			/* Send Request Sense command */
			if (cmdpt[0] != OP_S_RSENSE &&
			    scsipt_request_sense(devp, role,
						 sensept, senselen) &&
	...
	if ((sc_cmd.status_validity & 0x02) != 0) {
		if (sc_cmd.adapter_status != SC_GOOD_STATUS &&
			...
			(void) fprintf(errfp,
					"%s: %s %s:\n%s=0x%x %s=0x%x\n",
					APPNAME,
					"adapter status error on", devp->path,
					"Opcode", cmdpt[0],
					"Status", sc_cmd.adapter_status);
		}
		return FALSE;
	}
...
pthru_open(char *path)
{
	...
	if ((devp->fd = openx(path, O_RDONLY, NULL, SC_DIAGNOSTIC)) < 0) {
		DBGPRN(DBG_DEVIO)(errfp,
