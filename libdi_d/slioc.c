...
STATIC byte_t	slioc_sav_end_fmt,		/* Err recov saved end fmt */
       		scsipt_cdtext_buf[SZ_RDCDTEXT];  /* CD-Text Buffer */
...
slioc_disc_ready(curstat_t *s)
{
	...
	if (!app_data.cdtext_dsbl) {
		(void) scsipt_rdtoc(
			slioc_devp, DI_ROLE_MAIN, scsipt_cdtext_buf,
...
#ifdef _LINUX
	dt->load_cdtext = slioc_load_cdtext;
#else
	dt->load_cdtext = NULL;
#endif
 ...
 */
void
slioc_load_cdtext(curstat_t *s, di_cdtext_t *t)
{
	size_t			len;
	int			npacks,
				i,
