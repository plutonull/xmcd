 ...
 *
 *   IRIX mediad interaction added by Andrea Suatoni
 *   E-mail: a.suatoni@telefonica.net
 */
#ifndef lint
static char *_os_irix_c_ident_ = "@(#)os_irix.c	6.58 04/03/02";
#endif
...
)
{
	...
	dsreq.ds_sensebuf = (caddr_t) sensept;
	dsreq.ds_senselen = (uchar_t) senselen;
	dsreq.ds_time = (tmout ? tmout : DFLT_CMD_TIMEOUT) * 1000;

	/* Send the command down via the "pass-through" interface */
	if (ioctl(devp->fd, DS_ENTER, &dsreq) < 0) {
...
pthru_open(char *path)
{
	...
	/* Try to get an exclusive access id from mediad for this device */
	devp->val = mediad_get_exclusiveuse(path, "xmcd");
#ifdef IRIX_NEW_MEDIAD
	if (mediad_last_error() != 0)
		devp->val = -1;
#endif
...
{
	if (devp->fd > 0) {
		/* Release the exclusive use for this device */
		if (devp->val != -1)
			mediad_release_exclusiveuse(devp->val);
