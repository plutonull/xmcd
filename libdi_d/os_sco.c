 ...
 *   application to the SCO UNIX operating system.  The name "SCO" is
 *   are used here for identification purposes only.
 */
#ifndef lint
static char *_os_sco_c_ident_ = "@(#)os_sco.c	6.54 04/03/02";
#endif
...
#if defined(_SCO_SVR3) && defined(DI_SCSIPT) && !defined(DEMO_ONLY)
...
)
{
	...
	if (ioctl(devp->fd, SCSIUSERCMD, &sc) < 0) {
	...
	if (sc.host_sts || sc.target_sts) {
		if (app_data.scsierr_msg && prnerr) {
			(void) fprintf(errfp,
				    ...
				    "Opcode", cmdpt[0],
				    "Host_status", sc.host_sts,
				    "Target_status", sc.target_sts);
		}

		/* Send Request Sense command */
		if (sc.target_sts && cmdpt[0] != OP_S_RSENSE &&
...
pthru_open(char *path)
{
	...
	if ((devp->fd = open(path, O_RDONLY | O_NONBLOCK)) < 0) {
		DBGPRN(DBG_DEVIO)(errfp,
...
pthru_vers(void)
{
	return ("OS module for SCO UNIX");
}

#endif	/* _SCO_SVR3 DI_SCSIPT DEMO_ONLY */
